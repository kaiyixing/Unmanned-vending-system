...
package "用户访问层" {
    [Web用户端浏览器\nvending-web\n(Vue.js + Vite)] as Client
    [Web管理端浏览器\nvending-admin\n(Vue.js + Vite)] as Admin
}
...
package "后端服务层" {
    package "Spring Boot应用" {
        ...
        [取货码模块] as PickupModule
        [统计模块] as StatsModule
    }
    
    [MySQL主库\njdbc:mysql://mysql:3306\nvending_db] as MySQL
    [Redis缓存/锁\nredis://redis:6379] as Redis
}

package "存储层" {
    [MinIO对象存储\nminio://minio:9000\nBucket: vending-images] as MinIO
}

Client -down-> Nginx : HTTPS (443)
Admin -down-> Nginx : HTTPS (443)

Nginx -down-> UserModule : 内部HTTP
Nginx -down-> ProductModule
Nginx -down-> OrderModule
Nginx -down-> PayModule
Nginx -down-> InventoryModule
Nginx -down-> CabinetModule
Nginx -down-> RefundModule
Nginx -down-> PickupModule
Nginx -down-> StatsModule

UserModule -down-> MySQL
ProductModule -down-> MySQL
OrderModule -down-> MySQL
PayModule -down-> MySQL
InventoryModule -down-> MySQL
CabinetModule -down-> MySQL
RefundModule -down-> MySQL
PickupModule -down-> MySQL
StatsModule -down-> MySQL

UserModule -down-> Redis
ProductModule -down-> Redis
OrderModule -down-> Redis
InventoryModule -down-> Redis

ProductModule -down-> MinIO
CabinetModule -down-> MinIO
@enduml
```
